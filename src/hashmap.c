...
struct hashtable *
hashtable_create (size_t num_buckets)
{
    struct hashtable *table = (struct hashtable *)malloc (sizeof (hashtable));
    table->buckets = (struct bucket_item *)calloc (
        num_buckets, sizeof (struct bucket_item));
...
size_t
hash (struct hashtable *table, const char *word)
{
    size_t h = 0;
...
void
hashtable_print (struct hashtable *table)
{
...
void
hashtable_insert (struct hashtable *table, const char *word,
                  const size_t index)
{
    table->num_items++;
    ...
    if ((table->buckets[key]).next_item)
        {
            ...
            struct bucket_item *new_item
                = (struct bucket_item *)malloc (sizeof (struct bucket_item));
            new_item->word = malloc (sizeof (char) * strlen (word) + 1);
            memcpy (new_item->word, word, strlen (word) + 1);
...
int
hashtable_get (struct hashtable *table, const char *word)
{
    size_t key = hash (table, word);
    if ((table->buckets[key]).next_item)
        {
