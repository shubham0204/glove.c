 ...
 */
glove *
glove_create (const char *filename, const size_t vocab_size,
              const size_t embedding_dims)
{
    ...
    while (fscanf (file_ptr, "%s", &word_buffer[0]) == 1 && cntr < vocab_size)
        {
            float *embedding
                = (float *)malloc (sizeof (float) * embedding_dims);
            for (size_t i = 0; i < embedding_dims; i++)
                {
                    fscanf (file_ptr, "%f", (embedding + i));
                }
            char *word
                = (char *)malloc (sizeof (char) * strlen (word_buffer) + 1);
 ...
 */
int
glove_compare_cosine (glove *instance, const char *word1, const char *word2,
                      float *score)
{
 ...
 */
int
glove_compare_l2norm (glove *instance, const char *word1, const char *word2,
                      float *score)
{
    float *embedding1 = glove_get_embedding (instance, word1);
    float *embedding2 = glove_get_embedding (instance, word2);
    if (embedding1 && embedding2)
        {
            ...
            for (size_t i = 0; i < instance->embedding_dims; i++)
                {
                    diffs_squared
                        += powf (embedding1[i] - embedding2[i], 2.0f);
                }
